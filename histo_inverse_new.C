...
Bool_t b_invert_signal = kTRUE;// инверсия сигнала если выбирать  kTRUE если сигнал отрицательный(чтобы пьедестал был слева)
...
int maxoscnum = 20000; //чтобы не ждать очень долго обработки. Читается только это количество первых осциллограмм
...
void histo_inverse_new(const char* dirname,  int channel = 1, TString outfile = "histogram.root" ) //
{
   ...
   //нарисуем несколько гистограмм с осциллограммами для примера.
    ...
    for (long unsigned int i = 0; i < maxoscnum; i++) { 
	    ...
	    Double_t binPed1_m1 = 600.;
            Double_t binPed2_m1 = 720.;

            Double_t binSig1_m1 = 750.;
            Double_t binSig2_m1 = 870.;
	...
	c->cd(4);	
		...
		Double_t binPed1_m1Cor = 600.;
	        Double_t binPed2_m1Cor = 720.;

		Double_t binSig1_m1Cor = 750.;
	        Double_t binSig2_m1Cor = 870.;
		...
		//ped area
                ...
                //float pedCor = (maxped1Cor + maxped2Cor + maxped3Cor)/3.-p0Cor;
		...
		h->GetXaxis()->SetRangeUser(binSig1_m1Cor, binSig2_m1Cor);
                ...
                //float sigCor = (max1Cor + max2Cor + max3Cor)/3.-p0Cor;
